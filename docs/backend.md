...
HR 安排面试时间。状态：`INTERVIEW_PENDING` → `INTERVIEW_SCHEDULED`。
...
HR 标记面试已完成。状态：`INTERVIEW_CONFIRMED` → `INTERVIEW_DONE`。
...
## Module 5: Candidate Response (Public)

> 候选人通过 email 链接确认面试或请求改期。**不需登录**。

### `POST /candidates/respond/:id/confirm`
候选人确认参加面试。状态：`INTERVIEW_SCHEDULED` → `INTERVIEW_CONFIRMED`。
...
### `POST /candidates/respond/:id/reschedule`
候选人请求改期。状态：`INTERVIEW_SCHEDULED` → `INTERVIEW_RESCHEDULE_REQUESTED`。
...
## Module 6: Delete Candidate

### `DELETE /candidates/:id`
删除候选人及其 CV 文件。**HR only**。
